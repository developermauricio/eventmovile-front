    ...
    <div id="step01">
      ...
      <form @submit.prevent>
        ...
        <div class="col-md-12 text-right">
          <button  class="btn btn-primary" type="submit" @click.prevent="editUser">Guardar Cambios</button>
        </div>
...
export default {
    ...
    methods: {
      async getUser(){
        ...
        this.roleSelected = {id:this.user.role_id, name:this.user.rol}
        this.companySelected = {id:this.user.company.id, name:this.user.company.name}
        let label = document.getElementById('labelimageUser');
        label.innerText = this.user.pic
      },
      showCreateCompany(){
      ...
      async editUser(){
        ...
        dataUser.append('actived', this.user.actived)
        ...
        let response = await axios.post('users/'+this.user.id, dataUser).catch(error=>{
          this.$swal({icon:'error', text:error.response.data.error})
        })
        this.$swal('El usuario fue editado correctamente')
      },    
      getRoles(){
