<template>
    <div class="embed-responsive embed-responsive-16by9 " style="min-height:500px">
        ...
        <div class="agora-my-video">
            <StreamPlayer v-if="rendercurrentLocalStream && localScreen" :stream="localScreen" :domId="localScreen.getId()" ></StreamPlayer>
        ...
        <div class="agora-video embed-responsive-item" :key="index" v-for="(remoteStream, index) in remoteStreams">
            <StreamPlayer v-if="remoteStream.getId()" :stream="remoteStream" :domId="remoteStream.getId()"></StreamPlayer>
        </div>

        <div class="offset-1 col-10 offset-md-3 col-md-6 buttons-video-agora position-absolute " v-if="localStream">
            <div class="row ">
                <div class="col-4 m-0 p-0">
                    <button type="button" class="btn btn-primary w-100 h-100 rounded-0 m-0 p-0 border-0 text-lowercase" :class="classesButtons.video" @click="disableVideo()"><span class="ti-video-camera"></span>_{{classesButtons.messageVideo}}</button>
                </div>
                <div class="col-4 m-0 p-0">
                    <button type="button" class="btn btn-primary w-100 h-100 rounded-0 m-0 p-0 border-0 text-lowercase" :class="classesButtons.audio" @click="disableAudio()"><span class="ti-microphone-alt"></span>_{{classesButtons.messageAudio}}</button>
                </div>
                <div class="col-4 m-0 p-0">
...
export default {
    ...
    data() {
        return {
            ...
            classesButtons: {
            video:'enable-video',
            messageVideo:'habilitado',
            audio:'enable-audio',
            messageAudio:'habilitado',
            screen:'disable-screen',
            messageScreen:'No comparte',
            },
    ...
    methods: {
        ...
        enableShareScreen(){
            ...
            this.screen.joinChannel(this.option).then(() => {
                ...
                this.screen.shareScreen().then((stream) => {
                    ...
                    this.classesButtons.messageScreen = "Comparte"
        ...
        leaveScreen () {
            ...
            this.classesButtons.messageScreen = "No comparte"
            if(this.tmpStatusVideo)
        ...
        disableVideo(){
            if(this.localStream.disableVideo()){
                ...
                this.classesButtons.messageVideo = "Inhabilitado"
            } else {
                this.tmpStatusVideo = true
                this.localStream.unmuteVideo()
                this.classesButtons.video = 'enable-video'
                this.classesButtons.messageVideo = "Habilitado"
            }
        },
        disableAudio(){
            if(this.localStream.disableAudio()){
                ...
                this.classesButtons.messageAudio = "Inhabilitado"
            } else {
                this.localStream.unmuteAudio()
                this.classesButtons.audio = 'enable-audio'
                this.classesButtons.messageAudio = "Habilitado"
            }
        },
        getToken(){
            console.log("user_id token:",this.user.id)
            console.log("meeting_id token:",this.meetingId)
